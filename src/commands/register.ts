...
class Register extends Command {
  ...
  override async _handler({ args, db, io, signer }: StatesForHandler) {
    ...
    if (ri === undefined) {
      io.err(
        `Register: Invalid role "${role}", must be one of ${StrOfWalletRole.join(
          ', ',
        )}`,
      );
      return;
    ...
    if (failed.length === 0) {
      io.spinner(
        'register',
        `Successfully registered wallet ${wallet.address}`,
        SpinnerType.SUCCEED,
      );
      db.write();
      return;
    ...
    if (w === undefined)
      throw new Error('Register: failed to get first failing wallet');
    const wdb = (await db.getWallets({ address: w.address }))?.[0] ?? undefined;
    if (wdb === undefined)
      throw new Error('Register: failed to get matching wallet in db');
    // get existing in db
    ...
    if (
      ...
      io.spinner(
        'register',
        `Unpurged wallet ${wallet.address}`,
        SpinnerType.SUCCEED,
      );
      db.write();
      return;
